...
export async function createClientAccount(
  ...
  const res = await fetch('/api/create-user', {
    ...
    body: JSON.stringify({
      ...
      currencyCode, corporateGroup,
    }),
  });

  if (!res.ok) {
    const err = await res.json().catch(() => ({}));
    throw new Error(err.error || res.statusText || 'Error al crear la cuenta.');
  }
...
export async function getAllClientAccounts(): Promise<AppUser[]> {
  const { data: rows, error } = await supabase
    .from('profiles')
    .select('id, username, role, display_name, email, created_at, survey_permissions, logo_url, status, currency_code, corporate_group')
    .eq('role', 'client')
...
export async function updateClientProfile(
  ...
  updates: {
    ...
    permissions?: SurveyType[]; status?: string; currencyCode?: CurrencyCode;
